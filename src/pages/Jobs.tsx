...
const Jobs = () => {
  ...
  return (
    <div className="pb-16 min-h-screen bg-[#F7F8F8] bg-grid-pattern">
      ...
      <div className="bg-[#2B2D31] bg-grid-pattern-dark text-white py-12 sm:py-16 mb-12 border-b border-[#3F4147] relative overflow-hidden">
        <div className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-10 relative z-10 text-center">
          <div className="inline-flex items-center gap-2 px-3.5 py-1 rounded-full bg-[#35373C] border border-primary text-white text-xs font-bold tracking-wider uppercase mb-4 shadow-sm">
            Placement & Career Center 2026
          </div>
          <h1 className="text-3xl sm:text-5xl font-heading font-extrabold mb-4 text-white">
            Job Opportunities & Career Transformation
          </h1>
          <p className="text-neutral-300 max-w-3xl mx-auto text-sm sm:text-lg">
      ...
      <div className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-10">
        ...
        <div className="bg-primary text-white rounded-3xl p-8 sm:p-12 mb-16 shadow-xl relative overflow-hidden flex flex-col lg:flex-row items-center justify-between gap-8 border border-green-300/30">
          <div className="space-y-4 max-w-2xl">
            <span className="bg-white text-[#2B2D31] text-xs font-bold px-3.5 py-1 rounded-full uppercase tracking-wider inline-flex items-center gap-1.5 shadow-xs">
            ...
            <p className="text-green-50 text-sm sm:text-base leading-relaxed font-medium">
            ...
            <div className="flex flex-wrap gap-3 pt-2">
              {['Technical Training', '3-Month Internship', 'Interview Drills', 'Employer Hiring'].map((item, idx) => (
                <span key={idx} className="bg-white text-[#2B2D31] text-xs font-bold px-3.5 py-1.5 rounded-xl shadow-xs flex items-center gap-1.5">
          ...
          <div className="w-full lg:w-auto flex flex-col sm:flex-row gap-3">
            <a 
              ...
              className="bg-[#2B2D31] text-white hover:bg-[#35373C] border border-[#3F4147] font-heading font-extrabold px-8 py-4 rounded-xl text-center shadow-md transition-all text-sm"
            >
              Upload CV Now
            </a>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
          ...
          <div className="lg:col-span-7 space-y-6">
            <div className="flex items-center justify-between border-b border-neutral-200 pb-4">
              <h2 className="text-2xl font-heading font-extrabold text-[#2B2D31] flex items-center gap-2">
                <Briefcase className="w-6 h-6 text-primary" /> Active Job Vacancies
              </h2>
              <span className="text-xs bg-green-100 text-primary font-bold px-3.5 py-1 rounded-full border border-green-200">
                {liveJobs.length} Live Roles Available
              </span>
            </div>
            ...
            <div className="space-y-4">
              {liveJobs.map((job) => (
                <div key={job.id} className="bg-white p-6 rounded-3xl border border-neutral-200 shadow-sm hover:shadow-md transition-all space-y-4 group">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                    <div>
                      <h3 className="text-lg font-heading font-extrabold text-[#2B2D31] group-hover:text-primary transition-colors">
                        {job.title}
                      </h3>
                      <p className="text-xs font-bold text-neutral-500 flex items-center gap-1.5 mt-1">
                        <Building className="w-3.5 h-3.5 text-neutral-400" /> {job.company}
                      </p>
                    </div>
                    <span className="self-start sm:self-center bg-green-50 text-primary border border-green-200 text-xs font-extrabold px-3 py-1 rounded-full">
                      {job.type}
                    </span>
                  </div>

                  <div className="flex flex-wrap items-center gap-4 text-xs text-neutral-600">
                    <span className="flex items-center gap-1 font-medium"><MapPin className="w-3.5 h-3.5 text-neutral-400" /> {job.location}</span>
                    <span className="flex items-center gap-1 font-extrabold text-[#2B2D31]"><DollarSign className="w-3.5 h-3.5 text-primary" /> {job.salary}</span>
                  </div>
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-4 pt-3 border-t border-neutral-100">
                    ...
                    <button
                      ...
                      className="text-xs font-extrabold text-primary hover:text-primary-hover flex items-center gap-1 hover:underline"
                    >
                      Apply for this role
                    </button>
                  </div>
            ...
            <div className="mt-12 pt-8 border-t border-neutral-200">
              <h3 className="text-xl font-heading font-extrabold text-[#2B2D31] mb-6">How Euro Training Center Transforms Your Career</h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                {benefits.map((item, idx) => (
                  <div key={idx} className="bg-white p-6 rounded-2xl border border-neutral-200 shadow-xs space-y-2">
          ...
          <div id="cv-upload-section" className="lg:col-span-5 sticky top-24">
            <div className="bg-white p-8 rounded-3xl border border-neutral-200 shadow-xl space-y-6">
              <div className="border-b border-neutral-100 pb-4">
                ...
                <h2 className="text-2xl font-heading font-extrabold text-[#2B2D31]">
              ...
              {selectedJob && (
                <div className="bg-green-50 p-3.5 rounded-2xl border border-green-200 text-xs flex items-center justify-between">
                  <div>
                    <span className="font-bold text-neutral-600 block">Applying For Role:</span>
                    <span className="font-extrabold text-primary">{selectedJob}</span>
                  </div>
                  <button onClick={() => setSelectedJob('')} className="text-neutral-400 hover:text-neutral-700 font-bold text-xs">Clear</button>
                </div>
              )}

              {formSubmitted ? (
                <div className="bg-green-50 p-8 rounded-2xl text-center border border-green-200 space-y-4">
                  <CheckCircle className="w-12 h-12 text-primary mx-auto" />
                  <h3 className="text-lg font-heading font-extrabold text-[#2B2D31]">Application Received!</h3>
                ...
                <form onSubmit={handleApplySubmit} className="space-y-4">
                  ...
                  <div>
                    ...
                    <label className="w-full border-2 border-dashed border-neutral-200 hover:border-primary bg-[#F7F8F8] rounded-2xl p-6 text-center cursor-pointer flex flex-col items-center justify-center gap-2 transition-all group">
                      <div className="w-10 h-10 rounded-full bg-green-50 text-primary flex items-center justify-center border border-green-100">
                        {fileName ? <FileText className="w-5 h-5 text-primary" /> : <Upload className="w-5 h-5 text-primary" />}
                      </div>
                      <span className="text-xs font-extrabold text-[#2B2D31]">
                        {fileName ? fileName : "Click to select file or drag & drop"}
                      </span>
                      <span className="text-[10px] text-neutral-400 font-medium">Max file size: 10MB (Optional for initial inquiry)</span>
                      <input
                        type="file"
                        accept=".pdf,.doc,.docx"
                        onChange={(e) => {
                  ...
                  <button
                    type="submit"
                    className="w-full py-4 bg-primary hover:bg-primary-hover text-white font-heading font-extrabold text-sm rounded-xl shadow-md transition-all mt-2"
                  >
            ...
            <div className="mt-4 p-4 bg-[#2B2D31] text-neutral-300 border border-[#3F4147] rounded-2xl text-xs flex items-center justify-between shadow-sm">
              <div>
              ...
              <a href="tel:014975711" className="bg-primary hover:bg-primary-hover text-white px-3.5 py-2 rounded-xl font-mono font-extrabold transition-all shadow-xs">
                01-4975711
              </a>
            </div>
          </div>
